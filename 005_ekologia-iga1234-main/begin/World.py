...
class World(object):
	...
	def plague(self):
		print("Wszystkie organizmy tracą połowę życia.")
		self.organisms = [o for o in self.organisms if self.positionOnBoard(o.position)]
		for o in self.organisms:
	...
	def check(self,x,y):
		orgPosition = Position(xPosition = x, yPosition = y)
		isEmpty = self.getOrganismFromPosition(orgPosition)
		if not isEmpty:
			return True
		else:
	...
	def newOrg(self,organism, x, y):
		if organism == 'Grass':
			return Grass(position=Position(xPosition=x, yPosition=y), world=self)
		elif organism == 'Sheep':
			return Sheep(position=Position(xPosition=x, yPosition=y), world=self)
		elif organism == 'Antelope':
			return Antelope(position=Position(xPosition=x, yPosition=y), world=self)
		elif organism == 'Lynx':
			return Lynx(position=Position(xPosition=x, yPosition=y), world=self)


	def farFromLynx(self,fields,position):
		result = []
		for filed in fields:
			pomOrg = self.getOrganismFromPosition(filed)
			if isinstance(pomOrg, Lynx):
				a = position.x - filed.x
				b = position.y - filed.y
				if a > 0:
					position.x += 2
				elif a < 0:
					position.x -= 2
				if b > 0:
					...
					position.y -= 2
				pomPosition = self.getOrganismFromPosition(position)
				if pomPosition is None or isinstance(pomPosition, Plant):
					result.append(position)
					return result
				elif isinstance(pomOrg, Lynx):
					print(filed)
					result.append(filed)
					return result
